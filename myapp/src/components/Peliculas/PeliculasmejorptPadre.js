...
class PeliculasmejorptPadre extends Component {
  constructor() {
    super();
    this.state = {
      movies: [],
  ...
  componentDidMount() {
    fetch(
      "https://api.themoviedb.org/3/movie/top_rated?api_key=8a83423231f73046d3a699212802bf6e&language=en-US&page=" +
        this.state.num
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  cargarMas() {
    fetch(
      `https://api.themoviedb.org/3/movie/top_rated?api_key=8a83423231f73046d3a699212802bf6e&language=en-US&page=${this.state.num}`
    )
      .then((res) => res.json())
      .then((data) => {
        this.setState({
